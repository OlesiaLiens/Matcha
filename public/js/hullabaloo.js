...
//todo fatal errors in hullabaloo in actions (likes)
//todo когда наводишь на профиль юзера он быстро закрывается
//todo sort by age - неправильно работает (если есть ограниение макс 20 лет - показывает людей с возрастом больше 20)
//todo пофиксить хедер в Browse/ Search / Chat (ломаеться когда уменшаю окно браузера)
// todo добавить все нужные части хзедера на страницы
//todo пофиксить верстку в information (сделать адаптивной, поле city)
//todo пофиксить верстку в account  (отступ между information / Interests)
//todo можно выровнять фото на аккаунте + (у нас загружаються только png формат / jpg  - не обрабатьваеться)
//todo проблема с чатом (когда matched - как писать сообщения?)
// todo иногда search подвисает
